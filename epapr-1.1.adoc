...
.Example
<<device-tree-example>> shows an example representation of a simple device tree that
...
digraph tree
{
	...
	"cpu@0" [ label="<l>cpu@0\l|
		device_type = \"cpu\";\l
		reg = \<0\>;\l
		cache-line-size = \<32\>;\l
		cache-block-size = \<0x8000\>;\l
		clock-frequency = \<825000000\>;\l"
	];

	"memory@0" [ label = "<l>memory@0\l|
		reg = \<0 0x20000000\>\l"
	];

	"chosen" [ label = "<l>chosen\l" ];
	"aliases" [ label = "<l>aliases\l" ];
...
of characters in <<node-name-characters>>

[[node-name-characters]]
.Characters for node names
[option="header", cols="^10,30", width="200", align="center"]
|===
...
the set of characters in <<node-name-characters>>. The unit-address must match the
...
For example, in <<example-node-names>>, the device path to cpu #1 would be:
...
<<property-values-table>> describes the set of basic value types defined by the {spec}.
...
.Example
See the device tree fragment shown in <<address-size-cells-example>>.

[[address-size-cells-example]]
.Specifying number of address and size cells
----
FIXME
soc
...
In <<address-size-cells-example>>, the _#address-cells_ and _#size-cells_ properties of the soc
...
In <<address-translation-example>>, the ``soc`` node specifies a _ranges_ property of
...
of the properties described in <<cpu-node-properties>> are select standard properties
with specific applicable detail.

[[cpu-node-properties]]
.cpu node general properties
[options="header", cols="15,^10,^10,65"]
|===
|Property Name |Usage |Value Type |Definition
